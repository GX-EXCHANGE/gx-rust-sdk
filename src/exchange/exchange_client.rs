...
impl ExchangeClient {
    ...
    pub async fn usdc_transfer(
        ...
        let wallet = wallet.unwrap_or(&self.wallet);
        let gx_exchange_chain = if self.http_client.is_mainnet() {
            "Mainnet".to_string()
        } else {
            "Testnet".to_string()
        };
    ...
    pub async fn market_close(
        ...
        let base_url = match self.http_client.base_url.as_str() {
            "https://api.gx.exchange" => BaseUrl::Mainnet,
            "https://api.gx-exchange-testnet.xyz" => BaseUrl::Testnet,
...
mod tests {
    ...
    fn test_approve_builder_fee_signing() -> Result<()> {
        ...
        let mainnet_fee = ApproveBuilderFee {
            signature_chain_id: 421614,
            gx_exchange_chain: "Mainnet".to_string(),
